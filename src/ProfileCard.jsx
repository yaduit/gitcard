...
export default function ProfileCard({userData}) {
  ...
  <div className='flex flex-col lg:flex-row items-center lg:items-start justify-center gap-6 mt-1 cursor-pointer rounded-md w-full max-w-fit border border-gray-300 shadow-sm hover:shadow-md transition-shadow duration-200 bg-white p-4'>
    {/* Left Column - Image */}
    <div className='flex-shrink-0'>
      <img className='rounded-full w-48 h-48 shadow-md border-2 object-cover border-white mt-1' src={userData?.avatar_url || profileImage} alt="profile" />
    </div>
    
    {/* Right Column - Content */}
    <div className='flex flex-col items-center lg:items-start gap-4 lg:text-left'>
      ...
      {userData?.bio && (
        <p className='max-w-md text-center lg:text-left text-sm text-gray-600 leading-relaxed break-words line-clamp-3'>{userData?.bio || "Bio not available"}.</p>
      )}
      ...
      <div className='flex items-center gap-6'>
        ...
        {socials.portfolio && (
           ...
           < a href={socials.portfolio.startsWith('http') ? socials.portfolio : `https://${socials.portfolio}`}
            target="_blank"
